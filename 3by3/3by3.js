...
const desiredOrder = ["0", "1", "2", "3", "4", "5", "6", "7", "8"];
var bufferOrder = ["0", "1", "2", "3", "4", "5", "6", "7", "8"];
...
do {
    ...
    for (i = 1; i < bufferOrder.length; i++){
        for (j = 0; j < i; j++){
            if (randomOrder[i] < randomOrder[j] && randomOrder[i] != 8 && randomOrder[j] != 8){
                inversions++;
            }
        }
    }
...
function dragEnd() {
    if (!emptyTile.src.includes("8.jpg")) {
        return;
    }
